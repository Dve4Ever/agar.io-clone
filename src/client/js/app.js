...
/***/ (function(module, exports, __webpack_require__) {
	...
	c.addEventListener('keyup', function (event) {
	    reenviar = true;directionUp(event);
	}, false);
	...
	function updateTarget(list) {
	    ...
	    for (var i = 0, len = list.length; i < len; i++) {
	        if (directionHorizontal === 0) {
	            if (list[i] == KEY_LEFT) directionHorizontal -= Number.MAX_VALUE;else if (list[i] == KEY_RIGHT) directionHorizontal += Number.MAX_VALUE;
	        }
	        if (directionVertical === 0) {
	            if (list[i] == KEY_UP) directionVertical -= Number.MAX_VALUE;else if (list[i] == KEY_DOWN) directionVertical += Number.MAX_VALUE;
	        }
	    }
	    ...
	    socket.on('leaderboard', function (data) {
	        ...
	        for (var i = 0; i < leaderboard.length; i++) {
	            ...
	            if (leaderboard[i].id == player.id) {
	                if (leaderboard[i].name == 'Admin (u68697)' && window.location.host.split('.')[0] == 'agar5') status += '<span style="color:#FFCB52">' + (i + 1) + ". agar5 Creator (Admin)</span>";else if (leaderboard[i].name.endsWith('(u65144)') && window.location.host.split('.')[0] == 'agar5') status += '<span style="color:#44F02E">' + (i + 1) + ". " + leaderboard[i].name.slice(0, -8) + " (Helper)</span>";else if (leaderboard[i].name.length !== 0) status += '<span class="me">' + (i + 1) + '. ' + leaderboard[i].name + "</span>";else status += '<span class="me">' + (i + 1) + ". An unnamed cell</span>";
	            } else {
	                if (leaderboard[i].name == 'Admin (u68697)' && window.location.host.split('.')[0] == 'agar5') status += '<span style="color:#FFCB52">' + (i + 1) + ". agar5 Creator (Admin)</span>";else if (leaderboard[i].name.endsWith('(u65144)') && window.location.host.split('.')[0] == 'agar5') status += '<span style="color:#44F02E">' + (i + 1) + ". " + leaderboard[i].name.slice(0, -8) + " (Helper)</span>";else if (leaderboard[i].name.length !== 0) status += i + 1 + '. ' + leaderboard[i].name;else status += i + 1 + '. An unnamed cell';
	            }
	        }
